...
function Footer() {
    ...
    },
    {
      ...
      icon: (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
          <path fillRule="evenodd" clipRule="evenodd" d="M22,14.355c0-0.742-0.564-1.346-1.26-1.346H10.676c-0.696,0-1.26,0.604-1.26,1.346s0.563,1.346,1.26,1.346H20.74C21.436,15.702,22,15.098,22,14.355z" />
  ...
  return (
    <footer className="relative border-t border-white/5 mt-16">
      <div className="absolute inset-0 bg-gradient-to-t from-[#0b0b1a] to-transparent pointer-events-none" />
      <div className="max-width py-10 relative z-10">
          ...
          <nav className="flex flex-wrap justify-center gap-6">
            {navLinks.map(({ to, label }) => (
              <NavLink
                ...
                to={to}
                className={({ isActive }) =>
                  `text-sm transition-colors duration-200 ${isActive ? "text-white font-medium" : "text-slate-500 hover:text-slate-300"
                  }`
                }
              >
                {label}
        ...
        <div className="mt-8 pt-6 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-600">
          <span>© {year} Shashikumar Vanga. All rights reserved.</span>
          <span className="flex items-center gap-1">
            Built with
            <span className="text-gradient font-medium mx-1">React</span>
            &
            <span className="text-gradient font-medium mx-1">Tailwind</span>
          </span>
        </div>
      </div>
    </footer>
